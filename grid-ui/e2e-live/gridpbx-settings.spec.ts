...
test('manages an account-scoped Pivot profile without rendering private configuration', async ({
  ...
  await page.getByRole('button', { name: 'Create Pivot profile' }).click()

  const panel = page.getByRole('dialog', { name: 'Create Pivot profile' })
...
test('manages an account-scoped Webhook profile without rendering its private URL', async ({
  ...
  await expect(page.getByRole('heading', { name: 'Callflow integrations' })).toBeVisible()
  await page.getByRole('button', { name: 'Create integration' }).click()
  await page.getByRole('button', { name: 'Create Webhook profile' }).click()

  const panel = page.getByRole('dialog', { name: 'Create Webhook profile' })
...
test('manages Global and Account Carrier authorizations without exposing Switch identifiers', async ({
  ...
  await page.goto('/settings#callflow-integrations')
  await page.getByRole('button', { name: 'Create integration' }).click()
  await page.getByRole('button', { name: 'Create Global carrier profile' }).click()
  const globalPanel = page.getByRole('dialog', { name: 'Create Global Carrier profile' })
  await globalPanel.getByLabel('Profile name').fill('System carrier authorization')
  await globalPanel.getByRole('button', { name: 'Create profile' }).click()
  await expect(globalPanel).toBeHidden()
  await expect(page.getByText('Scope: global', { exact: true })).toBeVisible()

  await page.getByRole('button', { name: 'Create integration' }).click()
  await page.getByRole('button', { name: 'Create Account carrier profile' }).click()
  const accountPanel = page.getByRole('dialog', { name: 'Create Account Carrier profile' })
  ...
  await accountPanel.getByRole('button', { name: 'Create profile' }).click()
