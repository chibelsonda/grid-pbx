  ...
  actions: {
    ...
    async load(accountId: string, page = 1): Promise<void> {
      ...
      try {
        ...
        this.error = message(error, 'Unable to load menus.')
      } finally {
        this.loading = false
      }
    },
    async prepare(accountId: string, id?: string): Promise<void> {
      ...
      try {
        const [options, detail] = await Promise.all([
          menuApi.options(accountId),
          id ? menuApi.detail(accountId, id) : Promise.resolve(null),
        ])
        this.options = options
        this.detail = detail
      } catch (error) {
        this.error = message(error, 'Unable to prepare the menu form.')
      } finally {
        this.loading = false
      }
    },
    ...
    async save(accountId: string, input: MenuInput): Promise<boolean> {
      ...
      try {
        ...
        this.replace(
          isNew
            ? await menuApi.create(accountId, input)
            : await menuApi.update(accountId, this.detail!.id, input),
        )
        if (isNew) this.total += 1
        await this.load(accountId, this.page)
        return true
      } catch (error) {
        this.capture(error, 'Unable to save menu.')
        return false
      } finally {
        this.saving = false
      }
    ...
    async remove(accountId: string): Promise<boolean> {
      ...
      try {
        ...
        await menuApi.remove(accountId, id)
        ...
        this.capture(error, 'Unable to delete menu.')
        return false
      } finally {
        this.saving = false
      }
    ...
    async synchronize(accountId: string): Promise<void> {
      ...
      try {
        ...
        for (
          ...
          run = await menuApi.syncStatus(accountId, run.id)
        }
        if (run.status !== 'succeeded')
          throw new Error(run.error_message ?? 'Menu sync did not finish.')
        await this.load(accountId)
      } catch (error) {
        this.error =
          error instanceof Error ? error.message : message(error, 'Unable to synchronize menus.')
      } finally {
