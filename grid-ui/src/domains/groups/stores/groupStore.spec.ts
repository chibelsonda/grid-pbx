...
const options: GroupOptions = {
  users: [{ id: 'public-user', label: 'Ada', detail: '1001' }],
  devices: [],
  groups: [],
  media: [],
}
...
describe('group store', () => {
  ...
  it('loads account-scoped projected groups', async () => {
    ...
    expect(groupApi.list).toHaveBeenCalledWith('account-1', '', 1)
    expect(store.records).toEqual([record])
  })
  it('creates a group from public member references', async () => {
    vi.mocked(groupApi.options).mockResolvedValue(options)
    vi.mocked(groupApi.create).mockResolvedValue(record)
    const store = useGroupStore()
    await store.prepare('account-1')
    const input: GroupInput = {
      name: 'Support',
      music_on_hold_media_id: null,
      members: [{ type: 'user', id: 'public-user', weight: 1 }],
    }
    expect(await store.save('account-1', input)).toBe(true)
    expect(groupApi.create).toHaveBeenCalledWith('account-1', input)
