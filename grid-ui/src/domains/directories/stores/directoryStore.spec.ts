...
vi.mock('../api/directoryApi', () => ({ directoryApi: {
...
describe('directory store', () => {
  ...
  it('loads account-scoped projected directories', async () => {
    vi.mocked(directoryApi.list).mockResolvedValue({ data: [record], links: { first: null, last: null, prev: null, next: null }, meta: { current_page: 1, from: 1, last_page: 1, per_page: 25, to: 1, total: 1 } })
    const store = useDirectoryStore(); await store.load('account-1')
    expect(directoryApi.list).toHaveBeenCalledWith('account-1', '', 1); expect(store.records).toEqual([record])
  })
