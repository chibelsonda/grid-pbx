...
<template>
  <CrudSlideOver
    :title="record?.name ?? 'Media details'"
    eyebrow="GridPBX / Media"
    description="Projected metadata, protected audio, and known dependencies."
    width="medium"
    @close="emit('close')"
  >
    ...
    <div v-else-if="record" class="grid gap-5">
      ...
      <div class="grid gap-4 sm:grid-cols-2">
        <article class="card-surface p-4">
          <p class="text-[10px] font-bold tracking-wide text-slate-400 uppercase">Source</p>
          <p class="mt-2 text-sm font-semibold text-slate-700">
            {{ record.media_source ?? 'Unknown' }}
          </p>
        </article>
        <article class="card-surface p-4">
          <p class="text-[10px] font-bold tracking-wide text-slate-400 uppercase">Language</p>
          <p class="mt-2 text-sm font-semibold text-slate-700">
            {{ record.language ?? 'Not set' }}
          </p>
        </article>
        <article class="card-surface p-4">
          <p class="text-[10px] font-bold tracking-wide text-slate-400 uppercase">Content type</p>
          <p class="mt-2 text-sm font-semibold text-slate-700">
            {{ record.content_type ?? 'Not reported' }}
          </p>
        </article>
        <article class="card-surface p-4">
          <p class="text-[10px] font-bold tracking-wide text-slate-400 uppercase">Size</p>
          <p class="mt-2 text-sm font-semibold text-slate-700">{{ formattedSize }}</p>
        </article>
      </div>
        ...
        <div class="mt-4 grid grid-cols-3 gap-3 text-center">
          <div class="rounded-md bg-violet-50 p-3">
            <p class="text-lg font-semibold text-violet-700">
              {{ record.dependencies.music_on_hold }}
            </p>
            <p class="text-[9px] font-bold text-violet-500 uppercase">Hold setting</p>
          </div>
          <div class="rounded-md bg-blue-50 p-3">
            <p class="text-lg font-semibold text-blue-700">
              {{ record.dependencies.voicemail_greetings }}
            </p>
            <p class="text-[9px] font-bold text-blue-500 uppercase">Greetings</p>
          </div>
          <div class="rounded-md bg-amber-50 p-3">
            <p class="text-lg font-semibold text-amber-700">{{ record.dependencies.callflows }}</p>
            <p class="text-[9px] font-bold text-amber-500 uppercase">Callflows</p>
          </div>
        </div>
      </article>
      <div v-if="canManage" class="grid gap-3 border-t border-slate-200 pt-5 sm:grid-cols-3">
        ...
        <button
          type="button"
          class="inline-flex h-10 items-center justify-center gap-2 rounded-md border border-slate-200 bg-white text-xs font-semibold text-slate-600"
          @click="emit('replaceAudio')"
        >
          <ArrowPathIcon class="size-4" />Replace audio
        </button>
        <button
          type="button"
          class="inline-flex h-10 items-center justify-center gap-2 rounded-md border border-red-100 bg-red-50 text-xs font-semibold text-danger"
          @click="emit('delete')"
        >
