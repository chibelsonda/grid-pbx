...
import type { DeviceMetaflowModule, DeviceMetaflowNode } from './types/device'
...
export type MetaflowModuleDefinition = {
  value: DeviceMetaflowModule
  label: string
  description: string
  defaults: DeviceMetaflowNode['data']
...
export function metaflowDefinition(module: DeviceMetaflowModule): MetaflowModuleDefinition {
  return metaflowModules.find((item) => item.value === module) ?? metaflowModules[0]!
}

export function newMetaflowNode(module: DeviceMetaflowModule = 'transfer'): DeviceMetaflowNode {
