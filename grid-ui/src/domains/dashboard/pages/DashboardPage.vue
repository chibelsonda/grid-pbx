...
<template>
  ...
  <div class="mx-auto max-w-[1500px] p-4 sm:p-6 lg:p-8">
    ...
    <section class="mt-6 grid gap-6 lg:grid-cols-2">
      <article class="card-surface p-6"><p class="eyebrow">Current account</p><h2 class="mt-2 text-lg font-semibold text-slate-800">{{ accounts.selected?.name ?? 'No account selected' }}</h2><p class="mt-2 text-xs leading-5 text-slate-500">{{ accounts.selected ? `Realm: ${accounts.selected.realm ?? 'not supplied'}` : 'Map a Kazoo account in the API environment to begin.' }}</p><RouterLink to="/extensions" class="mt-5 inline-flex rounded-md bg-brand-500 px-4 py-2.5 text-xs font-semibold text-white">View extensions</RouterLink></article>
      <article class="card-surface p-6"><p class="eyebrow">Data flow</p><h2 class="mt-2 text-lg font-semibold text-slate-800">Kazoo remains the source of truth</h2><p class="mt-2 text-xs leading-5 text-slate-500">The Laravel worker copies selected operational fields into MySQL. This dashboard reads the projection for speed while writes will continue through audited Kazoo commands.</p><div class="mt-5 flex items-center gap-2 text-[11px] font-semibold text-emerald-700"><span class="size-2 rounded-full bg-success" /> API projection layer ready</div></article>
    </section>
  </div>
</template>
