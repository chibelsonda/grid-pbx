...
const form = reactive({
  ...
  should_block_anonymous: props.record?.should_block_anonymous ?? false,
  is_active: props.record?.is_active ?? false,
  numbersText: (props.record?.numbers ?? []).map((entry) => entry.number).join('\n'),
})
...
const invalidNumbers = computed(() =>
  numbers.value.filter((number) => !/^\+[1-9]\d{6,14}$/.test(number)),
)
function submit(): void {
  if (!invalidNumbers.value.length)
    emit('save', {
      name: form.name,
    ...
    <form class="grid gap-5" @submit.prevent="canManage && submit()">
        ...
        <article class="card-surface overflow-hidden">
          ...
          <div class="grid gap-4 p-5">
            ...
            <label class="grid gap-2"
              ...
              ><span v-if="fieldErrors.numbers" class="text-[10px] text-danger">{{
                fieldErrors.numbers[0]
              }}</span></label
            >
            <ToggleSwitch
      ...
      <div class="flex justify-end gap-3 border-t border-slate-200 pt-5">
        ...
        ><button
          v-if="canManage"
          type="submit"
          :disabled="saving || !form.name.trim() || invalidNumbers.length > 0"
