...
final class RecordingResourceClientTest extends TestCase
{
    ...
    public function test_it_paginates_bounded_recording_metadata(): void
    {
        ...
        parse_str($this->history[0]['request']->getUri()->getQuery(), $firstQuery); parse_str($this->history[1]['request']->getUri()->getQuery(), $secondQuery);
    ...
    private function client(array $responses, int $pageSize = 200): RecordingResourceClient
    {
        ...
        $switch = new SwitchClient(new Client(['handler' => $stack]), new SwitchConfig('http://switch.test/v2', 'unused'), new class implements TokenProvider { public function token(): string { return 'test-token'; } public function invalidate(): void {} });
        return new RecordingResourceClient($switch, $pageSize);
    }
    private function response(array $payload): Response { return new Response(200, [], json_encode($payload + ['status' => 'success'], JSON_THROW_ON_ERROR)); }
}
