...
use Generator;
use GridPbx\Switch\Dto\TemporalRuleSets\TemporalRuleSetSnapshot;
...
final readonly class TemporalRuleSetResourceClient
{
    ...
    public function allDetails(string $accountId): Generator
    {
        ...
        do {
            ...
            if (! is_array($data)) throw new InvalidSwitchPayloadException('Switch temporal rule set collection response data must be an array.');
            foreach ($data as $summary) {
                $id = is_array($summary) ? ($summary['id'] ?? null) : null;
                if (! is_string($id) || $id === '') throw new InvalidSwitchPayloadException('Switch temporal rule set collection entry must contain an id.');
                yield $this->get($accountId, $id);
            }
            $next = $payload['next_start_key'] ?? null;
            $cursor = is_string($next) && $next !== '' ? $next : null;
            if ($cursor !== null && isset($seen[$cursor])) throw new InvalidSwitchPayloadException('Switch temporal rule set pagination returned a repeated cursor.');
            if ($cursor !== null) $seen[$cursor] = true;
        } while ($cursor !== null);
    }

    public function get(string $accountId, string $setId): TemporalRuleSetSnapshot { return $this->snapshot($this->client->request('GET', $this->path($accountId, $setId))); }
    ...
    public function update(string $accountId, string $setId, TemporalRuleSetWriteData $set): TemporalRuleSetSnapshot
    {
        ...
        if ($snapshot->id !== $setId) throw new InvalidSwitchPayloadException('Switch temporal rule set response id does not match the requested resource.');
        return $snapshot;
    }
    public function delete(string $accountId, string $setId): void { $this->client->request('DELETE', $this->path($accountId, $setId)); }
    private function path(string $accountId, string $setId): string { return sprintf('accounts/%s/temporal_rules_sets/%s', rawurlencode($this->required($accountId, 'account')), rawurlencode($this->required($setId, 'rule set'))); }
    /** @param array<string, mixed> $payload */ private function snapshot(array $payload): TemporalRuleSetSnapshot
    {
        $data = $payload['data'] ?? null;
        if (! is_array($data)) throw new InvalidSwitchPayloadException('Switch temporal rule set response data must be an object.');
