...
namespace GridPbx\Switch\Domains\Recordings;

use Generator;
use GridPbx\Switch\Domains\Recordings\Dto\RecordingSnapshot;
...
final readonly class RecordingResourceClient
{
    ...
    {
        if ($this->pageSize < 1) {
            throw new InvalidArgumentException('Switch recording page size must be greater than zero.');
        }
    }
    ...
    public function all(string $accountId, int $createdFromUnix, int $createdToUnix): Generator
    {
        ...
        if ($createdFromUnix < 0 || $createdToUnix < $createdFromUnix) {
            throw new InvalidArgumentException('Switch recording time range is invalid.');
        }
        $cursor = null;
        $seen = [];
        do {
            ...
            if (! is_array($data)) {
                throw new InvalidSwitchPayloadException('Switch recording collection response data must be an array.');
            }
            foreach ($data as $recording) {
                if (! is_array($recording)) {
                    throw new InvalidSwitchPayloadException('Switch recording collection entries must be objects.');
                }
            ...
            if ($cursor !== null && isset($seen[$cursor])) {
                throw new InvalidSwitchPayloadException('Switch recording pagination returned a repeated cursor.');
            }
            if ($cursor !== null) {
                $seen[$cursor] = true;
            }
    ...
    public function get(string $accountId, string $recordingId): RecordingSnapshot
    {
        ...
        $data = $payload['data'] ?? null;
        if (! is_array($data)) {
            throw new InvalidSwitchPayloadException('Switch recording response data must be an object.');
        }
        $snapshot = new RecordingSnapshot($data);
        if ($snapshot->id !== $recordingId) {
            throw new InvalidSwitchPayloadException('Switch recording response id does not match the requested resource.');
        }
    ...
    public function delete(string $accountId, string $recordingId): void
    {
        $this->client->request('DELETE', $this->path($accountId, $recordingId));
    }

    private function path(string $accountId, string $recordingId): string
    {
        return sprintf('accounts/%s/recordings/%s', rawurlencode($this->required($accountId, 'account')), rawurlencode($this->required($recordingId, 'recording')));
    }
