...
namespace GridPbx\Switch\Domains\TemporalRules;

use Generator;
use GridPbx\Switch\Domains\TemporalRules\Dto\TemporalRuleSnapshot;
...
final readonly class TemporalRuleResourceClient
{
    ...
    public function allDetails(string $accountId): Generator
    {
        ...
        do {
            ...
            if (! is_array($data)) {
                throw new InvalidSwitchPayloadException('Switch temporal rule collection response data must be an array.');
            }
            foreach ($data as $summary) {
                $id = is_array($summary) ? ($summary['id'] ?? null) : null;
                if (! is_string($id) || $id === '') {
                    throw new InvalidSwitchPayloadException('Switch temporal rule collection entry must contain an id.');
                }
            ...
            if ($cursor !== null && isset($seen[$cursor])) {
                throw new InvalidSwitchPayloadException('Switch temporal rule pagination returned a repeated cursor.');
            }
            if ($cursor !== null) {
                $seen[$cursor] = true;
            }
    ...
    public function get(string $accountId, string $ruleId): TemporalRuleSnapshot
    {
        return $this->snapshot($this->client->request('GET', $this->path($accountId, $ruleId)));
    }
    ...
    public function delete(string $accountId, string $ruleId): void
    {
        $this->client->request('DELETE', $this->path($accountId, $ruleId));
    }

    private function path(string $accountId, string $ruleId): string
    {
        return sprintf('accounts/%s/temporal_rules/%s', rawurlencode($this->required($accountId, 'account')), rawurlencode($this->required($ruleId, 'rule')));
    }

    /** @param array<string, mixed> $payload */
    private function snapshot(array $payload): TemporalRuleSnapshot
    {
        $data = $payload['data'] ?? null;
        if (! is_array($data)) {
            throw new InvalidSwitchPayloadException('Switch temporal rule response data must be an object.');
        }
