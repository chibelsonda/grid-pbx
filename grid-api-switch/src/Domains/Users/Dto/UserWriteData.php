...
final readonly class UserWriteData
{
    public function __construct(
        ...
        public ?UserHotdeskData $hotdesk = null,
        public ?UserCredentialsData $credentials = null,
    ) {
    ...
    public function toSwitchData(): array
    {
        ...
        foreach (['email', 'timezone'] as $field) {
            $value = $this->{$field};
        ...
        $data = array_replace($data, $this->credentials?->toSwitchData() ?? []);

        if ($this->hotdesk !== null) {
            $data['hotdesk'] = $this->hotdesk->toSwitchData();
        }
