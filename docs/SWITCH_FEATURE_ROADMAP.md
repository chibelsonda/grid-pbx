...
| Group            | Group and ring-group configuration, membership, endpoints, and strategy                                                                     | Public-UUID Group/member relationships, Media mapping, cycle/dependency protection, external flags, and safe unknown full-update preservation are delivered                                                                                                                                                                                                                           | Foundation |
...
| Media            | Media metadata, prompts, music-on-hold, upload, and authorized streaming                                                                    | Schema-aligned upload metadata form, protected audio replacement/streaming, Headless UI music-on-hold choice, lossless server-side preservation of hidden and unknown safe public fields, and redacted detail delivered; generated TTS/recording remains capability-gated                                                                                                             | Foundation |
...
| Voicemail        | Mailbox CRUD, shared Basic/Advanced form, assignments, greetings, messages, and permitted lifecycle actions                                 | Project mailbox/message metadata; stream audio and keep PINs write-only                                                                                                                                                                                                                                                                                                               | Foundation |
...
existing dedicated operation gates remain the safe workflow. Focused package,
API, component, E2E TypeScript, and isolated headless checks passed without a
Phone Number or carrier mutation.
...
| IVR menus                          | CRUD, prompts, retries, timeout, key destinations                                                                                                                                                                                 | Menus, media, callflows                                                                | Complete installed form, Monster-aligned shared Basic/Advanced tabs, runtime-correct result-prompt suppression, write-only PIN replacement/removal, public Media UUID mapping, safe unresolved/unknown-field preservation, disposable synchronized lifecycle, dependency-safe delete, guided routing, and root-level DTMF/timeout branches delivered; deeper recursive editing remains planned                                                                                                                                                                                                                              | Foundation  |
| Time-of-day                        | Rules, holidays, rule sets, enable/disable/reset                                                                                                                                                                                  | Temporal rules and rule sets                                                           | Complete installed Rule fields, ordered public-UUID Rule Set membership, recursive PATCH preservation, explicit nullable-field clearing, projection/sync, safe deletion, effective status and audited controls, plus schema-correct `rule_set`/`_` guided routing delivered. Focused checks and a disposable Rule/Rule Set create/edit/reopen/control/delete lifecycle passed                                                                                                                                                                                                                                                                                   | Foundation  |
| Media and music on hold            | Upload, stream, rename, delete, assignment                                                                                                                                                                                        | Media and account settings                                                             | Validated upload/audio panels, protected range streaming, dependency-safe deletion, non-clipping account-default choice, hidden/unknown safe-field preservation, and metadata-only MySQL projection delivered. A disposable live upload/edit/delete lifecycle verified public/raw identity separation, nested unknown-field preservation, and zero active MySQL or Switch leftovers                                                                                                                                                                                                                                                                                | Foundation  |
| Directories                        | CRUD and user membership                                                                                                                                                                                                          | Directories and users                                                                  | Directory membership projection with public Extension UUID input, private User/Callflow mapping, external-flag retention, and safe unknown-field preservation across installed full-update behavior                                                                                                                                                                                                                                                                                                                                                                                                                                                                | Foundation  |
| Groups and ring groups             | CRUD, membership, endpoints, ring strategy                                                                                                                                                                                        | Groups, users, devices, callflows                                                      | Public-UUID Group/member relationships, private Switch-ID resolution, external-flag retention, safe unknown Group-field preservation, and Device-only bounded inline ring-group routing are delivered; broader recursive ring-group expansion remains capability-gated                                                                                                                                                                                                                                                                                                                                                                                             | Foundation  |
| Conferences                        | CRUD, role numbers, write-only multi-PIN replacement/removal, participant behavior, advanced named profile references, and runtime summary                                                                                         | Conferences and callflows                                                              | Normalized role-number projection, public Extension UUID/private owner-reference mapping, redacted source snapshot, Monster-aligned shared Basic/Advanced tabs, native recursive PATCH preservation for unknown/opaque fields and unchanged PINs, dependency-safe deletion, guided routing, and right-side panel delivered. Focused and disposable live create/edit/reopen/delete evidence passed; lock and participant mute/deaf/kick commands remain planned                                                                                                                                                                                                                     | Foundation  |
...
| Account voice settings             | Caller ID, timezone, language, music on hold, restrictions, recording defaults, dial plans, request formatters, preflow, metaflow activation/actions, and supported account defaults                                              | Accounts, media, phone-number classifiers, callflows, devices, extensions, and configs | Typed virtual settings from redacted `switch_json`; exact installed timezone/ringtone bounds; nullable inherited outbound privacy; safe regex rules; shared recursive action editor; recursive Account-PATCH plus locked-tree/unknown-option preservation; public UUID references; unresolved-reference controls; and protected storage URL preservation. Focused SDK/API/Zod/type/browser evidence passed in the 2026-08-31 drift re-audit; shared live Account mutation was intentionally avoided                                                                                                                                                                | Foundation  |
