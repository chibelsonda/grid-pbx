...
This document defines the Kazoo-backed capabilities planned for GridPBX. It is
...
The catalog was prepared from the locally available Kazoo Crossbar
...
target Kazoo deployment, security requirements, and delivery priority.
...
| Priority | Meaning |
| --- | --- |
| P0 | Platform foundation required by every Kazoo feature |
...
| Conditional | Implement only when the target Kazoo deployment supports it and the client confirms the need |
...
Every Kazoo feature follows these rules:
...
3. Kazoo remains authoritative for PBX configuration.
4. MySQL stores normalized, searchable projections of selected Kazoo data.
5. PBX writes go to Kazoo first and then update or invalidate the projection.
...
8. Credentials, authentication hashes, SIP passwords, API keys, and Kazoo
   tokens are not stored in projection payloads.
...
| Capability | Kazoo boundary | MySQL/application responsibility | Priority | Status |
| --- | --- | --- | --- | --- |
...
| Kazoo connectivity health | Crossbar root/auth request | Health state, failure reason, last successful check | P0 | Foundation |
...
- A failed Kazoo connection does not expose credentials or internal payloads.
...
into many live Kazoo calls on every page load.

### 5.2 People and extensions

An extension is a guided GridPBX workflow composed from several Kazoo
resources rather than a single Crossbar document.

| Capability | Kazoo resources | Projected data |
| --- | --- | --- |
...
| Bulk changes | Bulk or bounded individual Kazoo operations | Per-item result and reconciliation status |

The create/update workflow must report partial failures and support safe
recovery when one Kazoo resource succeeds and a later resource fails.
...
Primary Kazoo boundaries: devices, registrations, users, provisioner
...
Primary Kazoo boundaries: voicemail boxes, voicemail messages, users, media,
...
Primary Kazoo boundaries: phone numbers, callflows, number features, and
...
complete Kazoo callflow document editor.
...
Primary Kazoo boundaries: callflows, menus, temporal rules and rule sets,
users, devices, groups, voicemail boxes, media, and phone numbers.

## 6. P2 operational features

| Domain | User-facing capabilities | Kazoo boundary | Projection notes | Status |
| --- | --- | --- | --- | --- |
...
- Whether recordings stay in Kazoo storage or use an external object store
...
| White-labeling | Brand, domain, logo, colors, email identity | P3; split Kazoo and GridPBX ownership explicitly |
...
## 9. Business features adjacent to Kazoo
...
to be Kazoo-owned PBX features.
...
- Running or embedding Kazoo itself
...
- Direct database writes into Kazoo/CouchDB documents
...
A Kazoo feature can move to `Complete` only when:
...
5. Kazoo-first writes handle partial failure and schedule repair.
...
11. The client accepts the workflow against a representative Kazoo account.

## 12. Decisions required from the client

- Which P1 workflow must be delivered first after extensions
- Required Kazoo version and enabled Crossbar applications in production
