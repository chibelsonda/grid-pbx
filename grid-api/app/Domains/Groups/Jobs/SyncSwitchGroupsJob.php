...
namespace App\Domains\Groups\Jobs;
...
class SyncSwitchGroupsJob implements ShouldBeUnique, ShouldQueue
{
    ...
    {
        return "groups:{$this->switchAccountId}";
    }

    public function handle(GroupSynchronizationService $service): void
    {
    ...
    public function failed(?Throwable $exception): void
    {
        ...
        $message = mb_substr($exception?->getMessage() ?? 'The Switch group synchronization failed.', 0, 2000);
        ...
        SyncCheckpoint::query()->updateOrCreate(['switch_account_id' => $this->switchAccountId, 'resource_type' => 'groups'], ['last_sync_run_id' => $run->getKey(), 'status' => ProjectionStatus::Error, 'error_message' => $message]);
    }
}
