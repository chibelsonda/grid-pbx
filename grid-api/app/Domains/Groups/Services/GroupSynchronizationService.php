...
{
    public function __construct(private readonly SwitchGroupGateway $gateway, private readonly GroupProjectionService $projection, private readonly CallflowReferenceResolver $callflowReferences) {}
    ...
    public function handle(SyncRun $run): void
    {
        ...
        DB::transaction(function () use ($account, $run, $snapshots): void {
            ...
            $this->projection->reconcileNestedGroups($account);
            $missing = SwitchGroup::query()->where('switch_account_id', $account->getKey())->when($snapshots !== [], fn ($query) => $query->whereNotIn('switch_resource_id', array_keys($snapshots)))->get();
            SwitchGroup::destroy($missing->modelKeys());
            ...
            SyncCheckpoint::query()->updateOrCreate(['switch_account_id' => $account->getKey(), 'resource_type' => 'groups'], ['last_sync_run_id' => $run->getKey(), 'cursor' => null, 'status' => ProjectionStatus::Healthy, 'last_successful_at' => now(), 'error_message' => null]);
        });
    }
}
