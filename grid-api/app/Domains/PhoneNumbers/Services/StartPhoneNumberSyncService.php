...
class StartPhoneNumberSyncService
{
    public function handle(SwitchAccount $account, User $requestedBy): SyncRun
    {
        [$run, $created] = Cache::lock("switch-sync-start:{$account->getKey()}:phone_numbers", 10)
        ...
        if ($created) {
            SyncSwitchPhoneNumbersJob::dispatch((string) $run->getKey(), (string) $account->getKey());
        }
