<?php

namespace App\Domains\Voicemail\Models;

use App\Domains\Organizations\Models\SwitchAccount;
use App\Domains\SwitchSynchronization\Enums\ProjectionStatus;
use App\Shared\Models\Concerns\HasPublicUuid;
use Database\Factories\SwitchVoicemailMessageFactory;
use Illuminate\Database\Eloquent\Concerns\HasUlids;
use Illuminate\Database\Eloquent\Factories\HasFactory;
use Illuminate\Database\Eloquent\Model;
use Illuminate\Database\Eloquent\Relations\BelongsTo;
use Illuminate\Database\Eloquent\SoftDeletes;

class SwitchVoicemailMessage extends Model
{
    /** @use HasFactory<SwitchVoicemailMessageFactory> */
    use HasFactory, HasPublicUuid, HasUlids, SoftDeletes;

    protected $primaryKey = 'voicemail_message_id';

    protected $fillable = [
        'switch_account_id',
        'switch_voicemail_box_id',
        'switch_resource_id',
        'folder',
        'caller_id_name',
        'caller_id_number',
        'from_address',
        'to_address',
        'length',
        'source_timestamp',
        'occurred_at',
        'transcription_result',
        'transcription_text',
        'last_synced_at',
        'sync_status',
        'projection_version',
        'switch_json',
    ];

    /** @return BelongsTo<SwitchAccount, $this> */
    public function switchAccount(): BelongsTo
    {
        return $this->belongsTo(SwitchAccount::class, 'switch_account_id', 'account_id');
    }

    /** @return BelongsTo<SwitchVoicemailBox, $this> */
    public function voicemailBox(): BelongsTo
    {
        return $this->belongsTo(SwitchVoicemailBox::class, 'switch_voicemail_box_id', 'voicemail_box_id');
    }

    /** @return array<string, string> */
    protected function casts(): array
    {
        return [
            'length' => 'integer',
            'source_timestamp' => 'integer',
            'occurred_at' => 'datetime',
            'last_synced_at' => 'datetime',
            'sync_status' => ProjectionStatus::class,
            'projection_version' => 'integer',
            'switch_json' => 'array',
        ];
    }

    protected static function newFactory(): SwitchVoicemailMessageFactory
    {
        return SwitchVoicemailMessageFactory::new();
    }
}
