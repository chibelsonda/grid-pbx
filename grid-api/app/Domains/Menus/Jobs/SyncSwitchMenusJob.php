...
class SyncSwitchMenusJob implements ShouldBeUnique, ShouldQueue
{
    ...
    {
        return "menus:{$this->switchAccountId}";
    }

    public function handle(MenuSynchronizationService $service): void
    {
    ...
    public function failed(?Throwable $exception): void
    {
        ...
        $message = mb_substr($exception?->getMessage() ?? 'The Switch menu synchronization failed.', 0, 2000);
        ...
        SyncCheckpoint::query()->updateOrCreate(['switch_account_id' => $this->switchAccountId, 'resource_type' => 'menus'], ['last_sync_run_id' => $run->getKey(), 'status' => ProjectionStatus::Error, 'error_message' => $message]);
    }
}
