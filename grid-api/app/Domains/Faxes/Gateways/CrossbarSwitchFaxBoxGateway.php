...
class CrossbarSwitchFaxBoxGateway implements SwitchFaxBoxGateway
{
    ...
    public function all(SwitchAccount $account): Generator { foreach ($this->faxBoxes->allDetails($account->switch_account_id) as $box) yield $box->toArray(); }
    public function create(SwitchAccount $account, array $data): array { return $this->faxBoxes->create($account->switch_account_id, $this->writeData($data))->toArray(); }
    public function update(SwitchAccount $account, string $resourceId, array $data): array { return $this->faxBoxes->update($account->switch_account_id, $resourceId, $this->writeData($data))->toArray(); }
    public function delete(SwitchAccount $account, string $resourceId): void { $this->faxBoxes->delete($account->switch_account_id, $resourceId); }
