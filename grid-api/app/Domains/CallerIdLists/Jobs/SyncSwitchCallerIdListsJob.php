...
namespace App\Domains\CallerIdLists\Jobs;
...
class SyncSwitchCallerIdListsJob implements ShouldBeUnique, ShouldQueue
{
    ...
    {
        return "caller-id-lists:{$this->switchAccountId}";
    }

    public function handle(CallerIdListSynchronizationService $service): void
    {
    ...
    public function failed(?Throwable $exception): void
    {
        ...
        $message = mb_substr($exception?->getMessage() ?? 'The Switch Caller-ID List synchronization failed.', 0, 2000);
        $run->update([
        ...
        SyncCheckpoint::query()->updateOrCreate(
            ['switch_account_id' => $this->switchAccountId, 'resource_type' => 'caller_id_lists'],
            [
