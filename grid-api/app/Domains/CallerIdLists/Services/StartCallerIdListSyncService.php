...
class StartCallerIdListSyncService
{
    public function handle(SwitchAccount $account, User $requestedBy): SyncRun
    {
        [$run, $created] = Cache::lock("switch-sync-start:{$account->getKey()}:caller-id-lists", 10)
            ->block(3, function () use ($account, $requestedBy): array {
                $active = $account->syncRuns()
        ...
        if ($created) {
            SyncSwitchCallerIdListsJob::dispatch((string) $run->getKey(), (string) $account->getKey());
        }
