...
class ConferencePolicy
{
    ...
    public function view(User $user, SwitchConference $conference, SwitchAccount $account): bool { return $conference->switch_account_id === $account->getKey() && $this->viewAny($user, $account); }
    public function create(User $user, SwitchAccount $account): bool { return $this->access->canManageCallRouting($user, $account); }
    public function update(User $user, SwitchConference $conference, SwitchAccount $account): bool { return $conference->switch_account_id === $account->getKey() && $this->create($user, $account); }
    public function delete(User $user, SwitchConference $conference, SwitchAccount $account): bool { return $this->update($user, $conference, $account); }
