...
class ConferenceController extends Controller
{
    ...
    {
        /** @var User $user */ $user = $request->user(); $switchAccount = $accounts->findAccessible($user, $account);
        Gate::authorize('viewAny', [SwitchConference::class, $switchAccount]); $validated = $request->validated();
    ...
    {
        /** @var User $user */ $user = $request->user(); $switchAccount = $accounts->findAccessible($user, $account);
        Gate::authorize('viewAny', [SwitchConference::class, $switchAccount]); return response()->json(['data' => $conferences->options($switchAccount)]);
    }
    ...
    {
        /** @var User $user */ $user = $request->user(); $switchAccount = $accounts->findAccessible($user, $account); $model = $conferences->find($switchAccount, $conference);
        Gate::authorize('view', [$model, $switchAccount]); return new ConferenceResource($model);
    }
    ...
    {
        /** @var User $user */ $user = $request->user(); $switchAccount = $accounts->findAccessible($user, $account); Gate::authorize('create', [SwitchConference::class, $switchAccount]);
    ...
    {
        /** @var User $user */ $user = $request->user(); $switchAccount = $accounts->findAccessible($user, $account); $model = $conferences->find($switchAccount, $conference); Gate::authorize('update', [$model, $switchAccount]);
    ...
    {
        /** @var User $user */ $user = $request->user(); $switchAccount = $accounts->findAccessible($user, $account); $model = $conferences->find($switchAccount, $conference); Gate::authorize('delete', [$model, $switchAccount]);
