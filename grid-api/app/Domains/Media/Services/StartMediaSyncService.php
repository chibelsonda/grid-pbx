...
class StartMediaSyncService
{
    public function handle(SwitchAccount $account, User $requestedBy): SyncRun
    {
        [$run, $created] = Cache::lock("switch-sync-start:{$account->getKey()}:media", 10)
        ...
        if ($created) {
            SyncSwitchMediaJob::dispatch((string) $run->getKey(), (string) $account->getKey());
        }
