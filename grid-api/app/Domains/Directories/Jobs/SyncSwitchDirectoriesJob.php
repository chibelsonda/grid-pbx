...
namespace App\Domains\Directories\Jobs;
...
class SyncSwitchDirectoriesJob implements ShouldBeUnique, ShouldQueue
{
    ...
    {
        return "directories:{$this->switchAccountId}";
    }

    public function handle(DirectorySynchronizationService $service): void
    {
    ...
    public function failed(?Throwable $exception): void
    {
        ...
        $message = mb_substr($exception?->getMessage() ?? 'The Switch directory synchronization failed.', 0, 2000);
        ...
        SyncCheckpoint::query()->updateOrCreate(['switch_account_id' => $this->switchAccountId, 'resource_type' => 'directories'], ['last_sync_run_id' => $run->getKey(), 'status' => ProjectionStatus::Error, 'error_message' => $message]);
    }
}
