...
use App\Domains\Queues\Services\StartQueueSyncService;
...
class QueueSyncController extends Controller
{
    public function store(Request $request, string $account, SwitchAccountService $accounts, StartQueueSyncService $sync): JsonResponse
    {
        /** @var User $user */ $user = $request->user();
        $switchAccount = $accounts->findAccessible($user, $account);
        Gate::authorize('sync', [SwitchQueue::class, $switchAccount]);
    ...
    public function show(Request $request, string $account, string $run, SwitchAccountService $accounts): SyncRunResource
    {
        ...
        return new SyncRunResource($switchAccount->syncRuns()->where('resource_type', 'queues')->where('id', $run)->firstOrFail());
    }
}
