...
namespace App\Domains\Queues\Jobs;
...
class SyncSwitchQueuesJob implements ShouldBeUnique, ShouldQueue
{
    ...
    {
        return "queues:{$this->switchAccountId}";
    }

    public function handle(QueueSynchronizationService $service): void
    {
    ...
    public function failed(?Throwable $exception): void
    {
        ...
        $message = mb_substr($exception?->getMessage() ?? 'The Switch queue synchronization failed.', 0, 2000);
        ...
        SyncCheckpoint::query()->updateOrCreate(['switch_account_id' => $this->switchAccountId, 'resource_type' => 'queues'], ['last_sync_run_id' => $run->getKey(), 'status' => ProjectionStatus::Error, 'error_message' => $message]);
    }
}
