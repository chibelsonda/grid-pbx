...
class CrossbarSwitchBlacklistGateway implements SwitchBlacklistGateway
{
    ...
    {
        foreach ($this->blacklists->allDetails($account->switch_account_id) as $blacklist) {
            yield $blacklist->toArray();
        }
    }

    public function activeIds(SwitchAccount $account): array
    {
        return $this->accounts->account($account->switch_account_id)->blacklistIds;
    }

    public function create(SwitchAccount $account, array $data): array
    {
        return $this->blacklists->create($account->switch_account_id, $this->writeData($data))->toArray();
    }

    public function update(SwitchAccount $account, string $resourceId, array $data): array
    {
        return $this->blacklists->update($account->switch_account_id, $resourceId, $this->writeData($data))->toArray();
    }

    public function setActiveIds(SwitchAccount $account, array $resourceIds): void
    {
        $this->accounts->updateBlacklists($account->switch_account_id, new AccountBlacklistsWriteData($resourceIds));
    }

    public function delete(SwitchAccount $account, string $resourceId): void
    {
        $this->blacklists->delete($account->switch_account_id, $resourceId);
    }
