...
class TemporalRoutingSyncControllerTest extends TestCase
{
    use LazilyRefreshDatabase;

    public function test_operator_starts_temporal_routing_sync_and_receives_public_status(): void
    {
        Queue::fake([SyncSwitchTemporalRoutingJob::class]);
        [$user, $account] = $this->accessibleAccount();

        $response = $this->actingAs($user)
            ->postJson("/api/v1/accounts/{$account->id}/sync/temporal-routing");

        $response->assertAccepted()
            ->assertJsonPath('data.resource_type', 'temporal_routing')
        ...
        $this->assertDatabaseHas('switch_sync_runs', [
            ...
            'resource_type' => 'temporal_routing',
            'status' => SyncRunStatus::Queued->value,
        ]);
        Queue::assertPushed(
            SyncSwitchTemporalRoutingJob::class,
            fn (SyncSwitchTemporalRoutingJob $job): bool => $job->switchAccountId === (string) $account->getKey(),
        );

        $this->actingAs($user)
            ->getJson("/api/v1/accounts/{$account->id}/sync/temporal-routing/{$runId}")
            ->assertOk()
    ...
    public function test_read_only_user_cannot_start_temporal_routing_sync(): void
    {
        Queue::fake([SyncSwitchTemporalRoutingJob::class]);
