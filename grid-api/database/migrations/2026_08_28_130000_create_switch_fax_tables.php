...
return new class extends Migration
{
    public function up(): void
    {
        Schema::create('switch_fax_boxes', function (Blueprint $table): void {
            $table->bigIncrements('fax_box_id');
            ...
            $table->string('fax_header', 128)->nullable();
            $table->string('fax_identity', 64)->nullable();
            $table->string('fax_timezone', 64)->nullable();
            $table->unsignedTinyInteger('retries')->default(1);
            $table->boolean('t38_enabled')->default(false);
            $table->string('smtp_email_address')->nullable();
            $table->string('custom_smtp_email_address')->nullable();
            $table->json('smtp_permission_list')->nullable();
            $table->json('inbound_notification_emails')->nullable();
            $table->json('outbound_notification_emails')->nullable();
            ...
            $table->timestamps();
            $table->softDeletes();
            $table->unique(['switch_account_id', 'switch_resource_id'], 'sfb_account_resource_unique');
            $table->index(['switch_account_id', 'name'], 'sfb_account_name_index');
        });
        Schema::create('switch_faxes', function (Blueprint $table): void {
            $table->bigIncrements('fax_id');
            $table->uuid('id')->unique();
            $table->foreignId('switch_account_id')->references('account_id')->on('switch_accounts')->cascadeOnDelete();
            $table->foreignId('switch_fax_box_id')->nullable()->references('fax_box_id')->on('switch_fax_boxes')->nullOnDelete();
            $table->foreignId('switch_extension_id')->nullable()->references('extension_id')->on('switch_extensions')->nullOnDelete();
            $table->string('switch_resource_id');
            $table->string('fax_box_switch_resource_id')->nullable();
            $table->string('owner_switch_resource_id')->nullable();
            ...
            $table->string('to_name', 128)->nullable();
            ...
            $table->unsignedInteger('fax_speed')->default(0);
            ...
            $table->string('document_content_type', 128)->nullable();
            ...
            $table->index(['switch_account_id', 'folder', 'switch_created_at'], 'sfx_account_folder_created_index');
            $table->index(['switch_account_id', 'status'], 'sfx_account_status_index');
        });
    }

    public function down(): void
    {
        Schema::dropIfExists('switch_faxes');
        Schema::dropIfExists('switch_fax_boxes');
    }
};
