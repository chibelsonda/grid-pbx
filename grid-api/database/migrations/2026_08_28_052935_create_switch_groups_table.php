...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        Schema::create('switch_group_members', function (Blueprint $table): void {
            $table->ulid('group_member_id')->primary();
            $table->uuid('id')->unique();
            $table->foreignUlid('switch_group_id')->references('group_id')->on('switch_groups')->cascadeOnDelete();
            $table->foreignUlid('switch_extension_id')->nullable()->references('extension_id')->on('switch_extensions')->nullOnDelete();
            $table->foreignUlid('switch_device_id')->nullable()->references('device_id')->on('switch_devices')->nullOnDelete();
            $table->foreignUlid('nested_switch_group_id')->nullable()->references('group_id')->on('switch_groups')->nullOnDelete();
